...
instrument_positions = {
    ...
    "头部": ["生命值"],
    "手部": ["攻击力"],
    "躯干": ["未选择", "生命值%", "攻击力%", "防御力%", "暴击率", "暴击伤害", "治疗量加成", "效果命中"],
...
class RelicSimulator:
    ...
    def setup_window(self):
        ...
        # 构建图标路径（使用项目根目录下的图标.ico）
        icon_path = os.path.join(current_dir, '图标.ico')
        
        # 处理不同环境下的图标路径
        if hasattr(sys, '_MEIPASS'):  # PyInstaller环境
            icon_path = os.path.join(sys._MEIPASS, '图标.ico')
        elif os.path.exists(icon_path):  # 开发环境
            pass  # 使用已构建的路径
        else:  # Nuitka打包环境
